...
avg_path_full = '/labs/greene-lab/Shared_Tools/Group_Averages/ABCD_Combined_DISCandREP/ABCD_COMBINED_island_getaway_correction_with_SCAN.dlabel.nii'
...
FULL_NETWORKS = True # If True, uses avg_oath_full rather than avg_path
...
if not FULL_NETWORKS:
    ...
    avg_img = nib.load(avg_path_full)
    avg_data = avg_img.get_fdata().squeeze()

    label_ax = avg_img.header.get_axis(0)
    brain_ax = avg_img.header.get_axis(1)
    ...
    left_mask = brain_ax.name == 'CIFTI_STRUCTURE_CORTEX_LEFT'
    right_mask = brain_ax.name == 'CIFTI_STRUCTURE_CORTEX_RIGHT'

    network_indices = {}

    for network, label_id in base_labels.items():

        network_indices[f'{network}_left'] = np.where(
            (avg_data == label_id) & left_mask
        )[0]

        network_indices[f'{network}_right'] = np.where(
            (avg_data == label_id) & right_mask
        )[0]

        network_indices[f'{network}_full'] = np.where(avg_data == label_id)[0]

    for name in network_indices:
        network_indices[name] = network_indices[name][
            ~np.isin(network_indices[name], mtg_list)
        ]

    # define new network_labels dictionary for the full networks
    network_labels = {f'{network}_left': [label_id] for network, label_id in base_labels.items()}
    network_labels.update({f'{network}_right': [label_id] for network, label_id in base_labels.items()})
    network_labels.update({f'{network}_full': [label_id] for network, label_id in base_labels.items()})
...
if FULL_NETWORKS:
    results_df.to_csv("/labs/greene-lab/lab_members/emily/mtg/data/pMTG_FC_profiles_fullnetworks_meanFC.csv", index=False)
    print("Saved FC profiles to CSV (full networks).")
else:
